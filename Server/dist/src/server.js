...
import { initializeAdmin, loginAdmin, changeAdminCredentials, authenticateAdmin } from './admin_auth.js';
import cors from 'cors';
import { createProduct, getProductByBarcode, getAllProducts, updateProduct, deleteProduct, addInventoryBatch, updateInventoryBatch, deleteInventoryBatch, getProductStockLevel, reduceInventoryFIFO, createCustomer, getCustomers, updateCustomer, deleteCustomer, createSupplier, getSuppliers, updateSupplier, deleteSupplier, linkSupplierToProduct, createTransaction, getTransactionHistory, updateTransaction, deleteTransaction, getLowStockAlerts, getDailySalesTotal, getTransactionsBySupplier, getTransactionsByCustomer, getInventoryByProduct, getSuppliersByProduct, getAllSupplierProductLinks, getProductsBySupplier, updateSupplierProductPricing, unlinkSupplierFromProduct, getSalesTrends, getTopProducts, getInventoryTurnover, getProfitMargin, getCustomerLifetimeValue, getSupplierPerformance, getInventoryValuation, getPaymentAnalysis, getUserAnalytics } from './database_ops.js';
...
app.use(cors());
async function serveCustomFile(res, filePath) {
    try {
...
async function saveAllStatistics() {
    try {
        ...
        for (const user of users) {
            try {
                const analytics = await getUserAnalytics(user.folder_hash, user.id, user.shop_name);
...
function startStatsCollection(intervalHours = AUTO_SAVE_INTERVAL_HOURS) {
    if (statsInterval) {
...
app.post('/api/suppliers/link', authenticateToken, async (req, res) => {
    try {
        const { supplier_id, product_id, supplier_price, supplier_sku, min_order_quantity, lead_time_days, is_active } = req.body;
...
app.get('/api/analytics/sales-trends', authenticateToken, async (req, res) => {
    try {
        ...
        const trends = await getSalesTrends(req.user.folder_hash, period, startDate || defaultStartDate.toISOString().split('T')[0], endDate || defaultEndDate);
...
app.get('/api/analytics/top-products', authenticateToken, async (req, res) => {
    try {
        ...
        const topProducts = await getTopProducts(req.user.folder_hash, parseInt(limit), startDate, endDate);
...
app.get('/api/analytics/inventory-turnover', authenticateToken, async (req, res) => {
    try {
        ...
        const turnover = await getInventoryTurnover(req.user.folder_hash, productId ? parseInt(productId) : undefined);
...
app.get('/api/analytics/profit-margin', authenticateToken, async (req, res) => {
    try {
        ...
        const profitMargin = await getProfitMargin(req.user.folder_hash, startDate || defaultStartDate.toISOString().split('T')[0], endDate || defaultEndDate);
...
app.get('/api/analytics/complete', authenticateToken, async (req, res) => {
    try {
        const completeAnalytics = await getUserAnalytics(req.user.folder_hash, req.user.id, req.user.shop_name);
...
app.get('/api/analytics/dashboard', authenticateToken, async (req, res) => {
    try {
        const [lowStockAlerts, dailySales, paymentAnalysis, inventoryValuation] = await Promise.all([

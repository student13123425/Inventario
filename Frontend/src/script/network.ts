    ...
    return axiosInstance.post<SuccessResponse>('/api/suppliers/link', payload, { headers: getHeaders(token) })
      ...
      .catch(error => {
        if (axios.isAxiosError(error) && error.response) {
          ...
          throw new Error(data.error || 'Failed to link supplier to product');
        }
...
export function stockBoughtLast24hAnd30d(transactions: TransactionResponse[]): [number, number] {
  const now = Date.now();
  const ms24h = 24 * 60 * 60 * 1000;              // 86_400_000
  const ms30d = 30 * ms24h;                       // 2_592_000_000
  ...
  // expects the TransactionResponse type you provided
export function salesLast24hAnd7d(transactions: TransactionResponse[]): [number, number] {
  const now = Date.now();
  ...
  for (const t of transactions) {
    ...
    if (diff >= 0 && diff <= ms24h) {
      sum24 += t.amount;
      sum7 += t.amount; // also included in 7-day total
    } else if (diff > ms24h && diff <= ms7d) {
      sum7 += t.amount;
    }
  }

  // round to 2 decimals (useful for currency)
  const round = (v: number) => Number(v.toFixed(2));
